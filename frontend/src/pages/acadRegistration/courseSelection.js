...
const CourseSelection = () => {
  ...
  const fetchAllData = async () => {
    ...
    const getAvailableCourses = await axios.get("/student/courseregn", headers);
  ...
  const onSubmit = () => {
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          alert("You have successfully registered for the upcoming semester!");
        ...
        <div className="card-body">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            {availableCourses ? (
              ...
              <div>Courses not Available. Please try again later.</div>
            )}
            <button
              type="submit"
